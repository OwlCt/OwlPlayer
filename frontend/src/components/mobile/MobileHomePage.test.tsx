...
vi.mock('../../store/recentlyPlayedStore', () => ({
  useRecentlyPlayedStore: () => mocks.recentlyPlayedStore,
}));
...
vi.mock('./QuickAccessGrid', () => ({
  default: () => <div data-testid="quick-access-grid" />,
}));
...
describe('MobileHomePage', () => {
  beforeEach(() => {
    ...
    mocks.recentlyPlayedStore.fetchRecentlyPlayed.mockReset();
    mocks.playHistoryStore.topArtists = [];
    mocks.playHistoryStore.topTracks = [];
    mocks.recentlyPlayedStore.items = [];
  });
  ...
  it('hides monthly sections when there is no data', () => {
    ...
    expect(screen.queryByText('播放历史再积累一些，这里就会出现热门艺人。')).not.toBeInTheDocument();
    expect(screen.queryByText('多播放一些本地歌曲后，这里会出现你本月最常听的专辑。')).not.toBeInTheDocument();
    expect(screen.queryByText('多听几首本地歌曲后，这里会开始生成月度热门曲目。')).not.toBeInTheDocument();
  });
