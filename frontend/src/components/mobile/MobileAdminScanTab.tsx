...
export default function MobileAdminScanTab({ isAdmin }: MobileAdminScanTabProps) {
  const {
    overview,
    isLoading,
    isRefreshing,
  ...
  const disabled = !overview.enabled || overview.scan_running;
        ...
        <div className="space-y-8">
            ...
            actions={
              <div className="flex items-center gap-2">
                ...
                <button
                  ...
                  disabled={disabled || actionLoading === "scan:incremental"}
                  className="rounded-full border border-white/15 px-3 py-1.5 text-xs text-white active:bg-white/5 disabled:opacity-50"
                >
                  {actionLoading === "scan:incremental" ? "启动中" : "增量"}
                </button>
                <button
                  type="button"
                  onClick={() => runScan("full")}
                  disabled={disabled || actionLoading === "scan:full"}
                ...
                >
                  {actionLoading === "scan:full" ? "启动中" : "全量"}
                </button>
              </div>
            }
          >
            {!overview.enabled ? (
              <DisabledState
                reason={overview.disabled_reason || "本地媒体库未启用"}
              />
            ) : (
              <div className="space-y-4">
                ...
                <div className="divide-y divide-white/5 border-y border-white/5">
                  ...
                  <Row
                    label="文件丢失时"
                    value={
                      CLEANUP_POLICY_LABELS[overview.cleanup_policy] ||
                      overview.cleanup_policy
                    }
                  />
                  <Row
                    label="当前状态"
                    value={overview.scan_running ? "扫描中" : "空闲"}
                  />
                </div>
              </div>
            )}
          ...
          <Section title="最近扫描任务">
            {overview.recent_scan_tasks.length === 0 ? (
              <p className="text-sm text-white/55">暂无扫描历史</p>
            ) : (
...
function TaskCard({ task }: { task: LocalLibraryScanTask }) {
  ...
  return (
    <div className="space-y-2 rounded-md border border-white/5 bg-white/[0.02] p-4">
      <div className="flex items-center justify-between gap-3">
        <span
          className={`inline-flex rounded-full px-2.5 py-0.5 text-xs ${TASK_STATUS_CLASSES[task.status] || "bg-white/10 text-white/70"}`}
        >
          {TASK_STATUS_LABELS[task.status] || task.status}
        </span>
        <span className="text-xs text-white/45">
          {formatDateTime(task.started_at || task.created_at)} →{" "}
          {task.completed_at ? formatDateTime(task.completed_at) : "进行中"}
        </span>
      </div>
      <div className="text-sm text-white/85">{progress}</div>
      {task.last_error && (
        <div className="text-xs text-red-300">{task.last_error}</div>
      )}
